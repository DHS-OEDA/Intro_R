...
```{r}
# I can build this section out a bit more. Most data structures are the same but how they are evaluated by R changes under tidyverse. - Aaron. 
```
...
This is done using the `dplyr` and `tidyr` packages in the `tidyverse` collection. 
```{r}
# I can write this section - Aaron
```
